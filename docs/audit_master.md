...
4. **단계 4: 항목별 감사조서 및 감사절차 작성** — 🔶 부분 구현 (프로토타입)
   ...
   * **남은 작업**:
     - 감사조서 영속화 — 현재 `working_paper_md`는 매번 재계산되는 휘발성 데이터. `audit_working_papers` 테이블에 회사/연도/버전으로 저장 필요
...
|---|---|---|---|---|
| **P0** | 감사조서/분석 결과 영속화 — `audit_working_papers` 테이블(회사/연도/버전 + `draft→reviewed→approved` 상태) 신설 | 4단계 직접 해당, 5·6단계 선행조건 | ⏸ 미착수 | 저장 공간이 없으면 5·6단계는 이어붙일 결과물 자체가 없어 물리적으로 시작 불가 — 전체 로드맵의 선행 조건 |

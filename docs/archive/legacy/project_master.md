## 최근 업데이트 (2026-07-22) - 운영 로그인·마스터 탭·Notion 캘린더 안정화

- **로그인 DB 오류 해결**: Render의 `SUPABASE_KEY`가 anon 키로 설정되어 `users` 테이블 쓰기가 Supabase RLS 정책(`42501`)에 차단되던 문제를 확인했다. Render 환경변수를 service-role 키로 교체하고 재배포하여 로그인 및 사용자 조회를 정상화했다.
- **비밀번호 해시 호환성**: 신규 해시는 `pbkdf2:sha256:600000`으로 생성하고, 기존 Werkzeug scrypt 해시도 검증할 수 있는 호환 경로를 추가했다. 운영 마스터 계정의 비밀번호 설정 상태와 실제 로그인을 확인했다.
- **사이드바 탭 동기화**: 마스터 사이드바 클릭 시 선택 메뉴, 표시 화면, URL 해시, 브라우저 탭 제목이 즉시 함께 변경되도록 수정했다. 금융기관 조회·청구 관리 등 별도 클릭 핸들러가 이전 Mock/카드 화면을 남기던 중복 표시도 제거했다.
- **캘린더 변경 복원**: 별도 `agent/notion-calendar` 브랜치에만 있던 Notion 캘린더 커밋을 `main`에 통합했다. 월간 캘린더 UI와 `GET /api/master/calendar`가 운영에 반영되었다.
- **Render 운영 설정**: `NOTION_ACCESS_TOKEN`, `NOTION_TODO_DATABASE_ID`를 Render 환경변수에 등록하고 환경 갱신 배포를 완료했다. 운영 화면에서 2026년 7월 범위 **26개 일정 동기화**를 확인했다.
- **정적 자산 캐시 갱신**: `master.html`의 CSS/JavaScript URL에 버전 식별자를 추가하여 CDN·브라우저가 이전 탭 스크립트를 계속 사용하는 문제를 해결했다.
- **운영 검증**: `업무 요청 관리`, `금융기관 조회 관리`, `수수료 및 청구 관리`, `세무 일정 캘린더`를 순차 전환하여 화면 중복 없음, URL 해시 변경, 브라우저 제목 변경을 확인했다.
- **작업 원칙**: 정식 출시 전에는 `main`에 직접 반영한다. 새 브랜치가 불가피한 경우 사용자 승인을 먼저 받는다. 기능·운영 변경을 완료할 때 관련 `docs/*.md`를 함께 갱신하고 GitHub에 add·commit·push한다.

...
- **Drag & Drop 효과**: 파일 업로드 박스(`.mini-upload-box`) 위로 파일을 끌고 왔을 때(`.dragover`), 테두리가 오로라 포인트 색상(`--accent-color`)으로 빛나며 박스가 살짝 커지는 확대 스케일 애니메이션(`scale(1.02)`)으로 직관적 피드백을 제공합니다.

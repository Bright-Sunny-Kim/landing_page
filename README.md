...
- [x] **Phase 12. Master 포털의 수임사별 감사팀 편성 & 공인회계사 인력 풀(Workload) 대시보드 및 변경 이력 로깅 시스템 구축** (완료):
  - **감사인 인력 풀(Auditor Pool) & 4대 요약 통계 REST API (`blueprints/audit.py`)**:
    - `GET /api/audit/auditor-pool`: Supabase 및 사내 등록 공인회계사(CPA)/감사인 풀 조회 및 Datalist 자동완성 연동.
    - `GET /api/audit/assignment-summary`: 총 수임사, 실증감사 진행중, 투입 감사인 풀 수, 감사인별 주관(In-charge)/참여(Staff) Workload 매트릭스 실시간 집계.
    - `GET /api/audit/assignment-logs`: 감사팀 배정 생성/수정 시 Before ➔ After Diff 요약 및 변경자 로그 영구 보존 (`data/audit_assignment_logs.json`).
  - **Master 포털 배정 화면 전면 개편 (`templates/master.html`, `static/js/master_analytics.js`)**:
    - **4대 핵심 통계 카드 바**: 총 수임사, 진행중 감사, 투입 감사인 풀, 보고서 발행 완료 현황 상시 표출.
    - **감사인별 Workload 매트릭스 그리드**: 회계사별 주관/참여 수임사 뱃지 및 다중 배정 현황 시각화.
    - **실시간 검색 & 감사단계 필터 바**: 회사명/감사인 실시간 검색 및 기획/사전/실증/완결/발행 단계별 동적 필터링.
...
## 🏛️ [K-GAAP 감사조서 & AJE 수정분개 & 관련 손익항목 검토 시스템] (2026.09.25)

### 1. 주요 개발 및 개편 내역
1. **📝 조서 본체 공인회계사 K-GAAS / K-GAAP 표준 실무 문체 전면 개편**:
   - 감사 기본정보, 5대 경영진 주장(실재성, 완전성, 권리와 의무, 평가와 배분, 표시와 공시), 3대 실시간 대사표(총괄 대사, 세부 계정과목 Breakdown, 거래처 Top 5), 세부 실증절차 수행 내역, AJE 검토, 종합 감사 결론으로 체계화.
   - **마크다운 서식 실시간 렌더러 탑재**: `marked.js` 연동을 통해 조서 서식 미리보기(Preview), 마크다운 편집기(Markdown), K-GAAS 기준서 & RAG 가이드(Guide) 3대 탭을 자유롭게 전환 가능.
2. **⚖️ 감사 수정분개 (AJE / RJE: Adjusting Journal Entries) 인터랙티브 분개장 엔진**:
   - `계정과목 (차변)`, `금액 (차변)`, `계정과목 (대변)`, `금액 (대변)`, `내역(적요)`로 구성된 실시간 분개 입력 테이블 신설.
   - 대차평형 실시간 검증 뱃지(`🟢 대차일치 (차액 0원)` / `🔴 대차차액 발생`) 및 차·대변 합계 푸터 제공.
   - 계정과목 자동완성 Datalist를 탑재하여 현재 계정의 세부 과목 및 관련 손익 과목을 손쉽게 입력 가능.
3. **🔄 총괄 대사표 (Reconciliation Table) 실시간 자동 연동**:
   - AJE 분개장에 입력된 차변/대변 금액이 상단 계정과목별 세부 대사표의 `수정사항(차변)`, `수정사항(대변)`, `수정후금액` 및 총괄 요약 바에 100% 실시간 자동 반영.
4. **📊 관련 손익항목 검토 (Related P&L Items) 블록 신설**:
   - AJE/RJE 블록 아래 및 조서 서식 탭 위에 배치.
   - 선택된 B/S 계정(예: `G-0 유형자산`, `C-0 매출채권`, `A-0 금융상품` 등)과 직접 연계된 손익계산서(I/S) 주요 손익항목(감가상각비, 대손상각비, 이자수익/비용, 처분손익 등)을 6대 장부에서 자동 매핑·추출.
   - AJE 분개장에서 손익 계정 분개 입력 시, 관련 손익항목의 차변/대변 수정사항 및 `수정후 최종손익`과 `손익 합계`에 실시간 연동 반영.
5. **고객사 선택 드롭다운 목록 안정화 및 웹 UI 최적화**:
   - MinIO S3 오브젝트 스토리지, 로컬 아카이브, 감사 배정 목록을 통합하여 전사 감사 대상 기업 목록 정상 표출.
   - PC/데스크톱 웹 브라우저(769px 이상)에서 모바일 헤더가 불필요하게 노출되지 않도록 `display: none !important;` 반응형 처리.

---

---

## 🏛️ [마스터 회계감사통제 포털 개편 및 3대 서브탭 연계 시스템] (2026.09.26)

### 1. 주요 개발 및 개편 내역
1. **🏷️ 메뉴/페이지명 개편**:
   - `회계감사` ➔ **`회계감사통제`**로 명칭 변경 및 불필요한 구 탭(금융기관조회 관리, 감사 증빙 문서 관리) 전면 제거.
2. **👥 1. 감사인 인력 풀 관리 탭 (`#subtab-auditor-pool`)**:
   - Supabase `users` 테이블(`is_auditor=true` 또는 `task_type='회계감사'`, `role IN ('cpa', 'auditor', 'master')`) 기반 실시간 인력 풀 조회.
   - **서버 인메모리 5분 TTL 캐싱 & Stale-While-Revalidate 로컬스토리지 2단계 캐시**: 불필요한 부하 없이 0.001초 만에 즉시 표출.
   - **`🏢 N개 수임사` 팝오버 모달 (`#modal-auditor-assigned-detail`)**: 감사인별 배정 수임사 및 K-GAAP 105개 절차 중 배정된 구체적 절차 리스트 즉시 조회.
3. **🏢 2. 감사대상회사현황 탭 (`#subtab-audit-target-companies`)**:
   - 주요업무가 `회계감사`로 등록된 고객사의 연도별 서류 제출율(프로그레스 바), 6대 장부 수집 상태(6/6 완비 뱃지), 감사 진행 단계, 주임 회계사(In-charge) 모니터링 테이블.
   - 테이블 스크롤 시 헤더 고정(`position: sticky; z-index: 10`) 및 한 줄(`nowrap`) 가독성 레이아웃 최적화.
4. **📋 3. 회사별 감사팀 배정 (Job Assign) 탭 (`#subtab-audit-assign`)**:
   - 상단 회사명 및 사업연도 Dropdown 선택 컨트롤 배치.
   - **감사인별 진행률 카드 그리드**: In-charge 및 배정된 감사인별 절차 개수 및 진행률(%) 실시간 계산.
   - **전체 105개 절차 매트릭스 (5개 행 뷰포트 + 세로 스크롤)**: K-GAAP 1000~8000 전 절차별 담당 감사인 인라인 셀렉트 및 `[💾 배정 저장]` 영구 보존.
   - **미지정 절차 `cpaeastsun@gmail.com` 자동 지정 안전 폴백**.
5. **🔗 3대 서브탭 간 원클릭 딥링크(Deep Link) & Focus View 연계**:
   - 인력 풀 팝오버 내 `[🏢 (주)회사명 배정 관리로 이동 ›]` 클릭 시 배정 탭으로 즉시 전환, 회사/연도 자동 선택 및 해당 감사인 담당 절차 Focus View 실행.
   - 배정 탭 3번 블록의 감사인 카드 클릭 시 4번 테이블에서 해당 감사인의 담당 절차만 집중 필터링.
   - 배정 저장 시 백엔드 캐시 자동 무효화(`invalidate_auditor_pool_cache()`) 및 3대 탭 실시간 0초 동기화.
...
- **정규화 RDBMS 마이그레이션 (`audit_engagements` & `audit_procedure_assignments`)**:
  - 현재 파일 기반(`data/audit_assignments.json`) 배정 데이터를 Supabase PostgreSQL 관계형 DB로 완전 이관하여 외래키(FK) 참조 무결성 및 동시성 락(Row-level Lock) 보장.
- **WebSocket / Server-Sent Events 기반 실시간 멀티 세션 동기화**:
  - 다른 관리자 또는 참여 감사인이 배정을 변경하거나 업무를 완료했을 때 새로고침 없이 3대 서브탭의 배정 수치와 프로그레스 바가 실시간으로 푸시 동기화되는 기능 구현.
- **감사인 인력 풀 ↔ 회사별 감사팀 배정 ↔ 감사대상회사 다차원 필터링 & 매트릭스 뷰**:
  - 타겟 회사 탭에서 특정 회사를 클릭했을 때 Job Assign 탭으로 직결되는 양방향 컨텍스트 스위칭 및 감사인별 누적 워크로드(Overload Warning) 과부하 방지 알림 위젯 구축.
- **감사 절차별 실시간 진척도(체크리스트/조서 생성 여부) 자동 연동**:
  - 회계사가 `/audit` 포털에서 실제 조서를 작성·저장하면 Job Assign 탭의 절차 진행 상태가 `진행중 ➔ 완료`로 자동 변경되어 진행률이 실시간 갱신되는 완전 자동화 파이프라인 완성.

### 📌 과제 2: Audit 페이지의 수정전 vs 수정후 재무제표 비교 팝업 기능 추가
- **재무제표 수정비교 대형 모달 (Side-by-side Diff Viewer)**:
  - 감사조서 및 AJE 분개 작성 중 [📊 재무제표 수정비교] 버튼을 클릭하여 `수정전 재무상태표/손익계산서`와 `AJE 수정사항 반영후 최종 재무제표`를 좌우 2단으로 직관적 비교.
- **주요 재무 지표 변동 효과 하이라이트**:
  - 자산총계, 부채총계, 자본총계, 매출액, 영업이익, 당기순이익의 AJE 수정 전/후 증감액 및 변동률 자동 산출 및 시각적 강조.
  - 대차평형 상태 재검증 및 감사보고서 수치와의 일치성 확인 지원.

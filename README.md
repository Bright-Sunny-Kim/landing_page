...
- [ ] **1. 파싱 데이터 정규화 영속화 및 다각적 분석용 로드(Load) 허브 구축 (최우선 과제)**:
  - **① 정규화된 파싱 데이터 구조적 DB 저장 (Parsed Financial Data Repository)**:
    - 엑셀 파일을 매번 다시 업로드/파싱할 필요 없이, 최초 1회 파싱된 5대 회계 데이터를 정규화된 테이블(`parsed_balance_sheets`, `parsed_income_statements`, `parsed_trial_balances`, `parsed_journal_entries`, `parsed_subledgers`)에 구조화하여 안전하게 저장
    - 대용량 분개장(수십만 행)과 거래처원장 데이터를 고속 인덱싱하여 영구 보관
  - **② 다목적 분석용 파싱 자료 원클릭 불러오기 & 결합 (Ad-hoc Analysis & Cross-loading)**:
    - 과거에 업로드/파싱해 둔 특정 기업·특정 연도의 데이터를 즉시 불러와 다양한 분석 모듈로 즉각 전달
    - **시계열 다개년 추세 분석 (Multi-year Longitudinal Trend)**: 3~5개년도 파싱 자료를 일괄 결합하여 매출·수익성·원가율·차입금의 다개년 변동 추이 및 계절성 파악
    - **동종업계 피어 그룹 교차 비교 (Cross-sectional Peer Benchmarking)**: 동일 업종 내 여러 고객사의 파싱 데이터를 불러와 재무건전성, 마진율, 매출채권 회수속도 상대 비교
    - **심층 포렌식 및 세무조정 연계 (Deep Forensic & Tax Adjustment)**: 저장된 분개장 데이터를 바탕으로 접대비 한도초과, 가지급금 인정이자, 감가상각비 세무조정용 기초 데이터 자동 산출
  - **③ 커스텀 피벗 및 다차원 조건 검색 뷰어 (Interactive Pivot & Filter Explorer)**:
    - 저장된 분개/원장 데이터를 사용자가 원하는 조건(계정과목별, 거래처별, 월별, 금액 구간별, 적요 키워드별)으로 자유롭게 슬라이싱/다이싱하여 조회하는 인터랙티브 탐색기 지원
- [ ] **2. DART 전자공시 API 실시간 대사**: 외감/상장 기업의 경우 DART 공시 재무제표와 업로드 엑셀 간의 자동 대사 기능 결합
- [ ] **3. 국세청 홈택스 전자세금계산서 스크래핑 연동**: 세금계산서 합계표와 장부상 외상매출/매입금 간의 실시간 불부합 검증
- [ ] **4. PDF 감사 보고서 익스포트**: 마크다운 뷰어 내용을 회계법인 표준 양식의 스타일링된 PDF로 1초 출력
- [ ] **5. 벤포드의 법칙(Benford's Law) 추가**: 분개장 금액의 첫째 자리 수 분포 분석을 통한 인위적 조작 전표 고도화 탐지

...
### 📌 과제 1: Master 페이지의 회사별 감사팀 배정 페이지 고도화
- **수임 회사별 감사팀 편성 관리**:
  - 회사별 주관 공인회계사(In-charge), 참여 회계사(Staff), 업무수행이사(Engagement Partner) 지정 및 변경 기능.
  - 감사 일정(기초재고실사, 기말감사, 보고서발행) 단계별 투입 리소스 및 상태 관리.
- **감사인 인력 풀 및 배정 현황 시각화**:
  - `is_auditor` 권한 보유 회계사들의 직급/전문분야별 필터링, 팀원별 다중 수임사 배정 현황 대시보드.
  - 감사팀 배정 이력 및 변경 로그 영구 보존.
